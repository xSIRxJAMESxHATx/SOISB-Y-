"""
Mount Rushmore composite using real mountain photo base.
Player portraits are rock-toned and blended into the four carved niches.
"""
...
BASE_CANDIDATES = [
    Path(__file__).resolve().parent.parent / "assets" / "mount_rushmore_base.jpg",
    Path(__file__).resolve().parent.parent / "assets" / "mount_rushmore_base.png",
]
...
def _rock_blend_face(im: Image.Image, size: int) -> Image.Image:
    """Stone-carved look: desaturate, warm granite tint, soft oval."""
    im = im.convert("RGBA")
    # Focus on upper face if portrait is full body-ish
    w, h = im.size
    if h > w * 1.15:
        im = im.crop((0, 0, w, int(w * 1.15)))
    ...
    mask = mask.filter(ImageFilter.GaussianBlur(max(2, size // 40)))
    rock.putalpha(mask)
    return rock
...
def generate_rushmore(players: List[str], title: str = "Mount Rushmore") -> Image.Image:
    ...
    for i, (x, y, sz) in enumerate(slots):
        ...
        # light multiply-style blend: paste with alpha onto mountain
    ...
    # title bar
    draw.rectangle([0, 0, W, int(H * 0.07)], fill=(15, 20, 30, 180))
    ...
    for i, (x, y, sz) in enumerate(slots):
        ...
        ty = min(y + sz + 8, H - 30)
        draw.rounded_rectangle([x, ty, x + sz, ty + 26], radius=6, fill=(20, 18, 15, 210))
        draw.text((x + sz // 2, ty + 13), label, fill=(245, 240, 230, 255), font=font_name, anchor="mm")

    draw.rectangle([0, H - 28, W, H], fill=(15, 20, 30, 160))
    draw.text(
        (W // 2, H - 14),
        "Fan composite on public-domain mountain photo · Not official · SO!SB!Y!",
...
def rushmore_to_bytes(players: List[str], title: str = "Mount Rushmore") -> bytes:
    try:
        ...
        d = ImageDraw.Draw(img)
        d.text((480, 320), "Rushmore unavailable — try again", fill=(255, 255, 255), anchor="mm")
    buf = io.BytesIO()
